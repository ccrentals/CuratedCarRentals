...
import { selectAndUploadDirectImages, type DirectUploadProgress } from "@/lib/uploads/directUpload-client";
  ...
  const handleUpload = async () => {
    ...
    try {
      const nextUrls = await openUploadcareImagesDialog({
        ...
        onProgress: (progress) => {
          const phase = progress.phase === "authorizing" ? "Checking" : progress.phase === "finalizing" ? "Saving" : "Uploading";
          setMessage(`${phase} ${progress.fileIndex + 1} of ${progress.fileCount}: ${progress.fileName}${progress.phase === "uploading" ? ` (${progress.percent}%)` : ""}`);
        },
      });
