...
export async function POST(request: Request) {
  ...
  if (
    (purpose === "LANDING_CONTENT" || purpose === "VEHICLE_GALLERY") &&
    !hasRequiredAdminAccess(auth.actor.role, "admin")
  ) {
    return json({ ok: false, error: "Administrator access is required for this image purpose." }, 403);
  }
  ...
  const checksum = normalizeSha256(rawChecksum);
  if (!checksum) return json({ ok: false, error: "A valid image checksum is required." }, 400);
