...
function jsonError(status: number, code: string, error: string, debug?: Record<string, unknown>) {
...
export async function POST(request: Request) {
  ...
  for (const key of requiredEnv) {
    if (!process.env[key]) {
      logWarn("wipay_custom_start_missing_env", { missing: key });
  ...
  if (!/^\d+$/.test(accountNumber)) {
    logWarn("wipay_custom_start_invalid_account_number");
  ...
  if (!["sandbox", "live"].includes(process.env.WIPAY_ENV ?? "")) {
    logWarn("wipay_custom_start_invalid_env", { env: process.env.WIPAY_ENV });
  ...
  const requestedAmount = parseAmount(body?.customAmountCents ?? body?.amountCents);
  ...
  if (summary.balanceDue <= 0) {
    return jsonError(409, "already_paid", "Already fully paid");
  }

  const fallbackAmount = parseAmount(pricing.custom_payment_amount_cents);
  const amountToCharge = requestedAmount ?? fallbackAmount;
  if (!amountToCharge || amountToCharge <= 0 || amountToCharge > summary.balanceDue) {
    return jsonError(
      ...
      { requestedAmount, fallbackAmount, balanceDue: summary.balanceDue },
    );
  }

  const orderId = buildOrderId();
  const totalDecimal = formatJmdDecimal(amountToCharge);
  ...
  try {
    await dbQuery("update bookings set payment_option = 'CUSTOM', custom_payment_amount_cents = $2, pricing_json = $3, updated_at = now() where id = $1", [
      booking.id,
      amountToCharge,
      {
        ...pricing,
        payment_option_selected: "CUSTOM",
        custom_payment_amount_cents: amountToCharge,
    ...
    const insertResult = await dbQuery(
      ...
      [
        ...
        orderId,
        {
          bookingId: booking.id,
          payment_type: "custom",
          custom_amount_cents: amountToCharge,
    ...
    paymentId = (insertResult.rows[0] as { id?: string } | undefined)?.id ?? null;
  } catch (error) {
    const message = error instanceof Error ? error.message : "Insert failed";
    logError("wipay_custom_start_db_insert_failed", error, { bookingId: booking.id, orderId });
  ...
  try {
    ...
    return NextResponse.json({ ok: true, redirectUrl: wipayResponse.url, paymentId });
  } catch (error) {
    ...
    try {
      ...
      logError("wipay_custom_start_db_update_failed", dbError, {
        bookingId: booking.id,
        orderId,
        paymentId,
      });
    ...
    logError("wipay_custom_start_request_failed", error, {
      bookingId: booking.id,
      orderId,
      amountDecimal: totalDecimal,
      env: process.env.WIPAY_ENV ?? "sandbox",
