...
export async function POST(request: Request) {
  ...
  const requiredEnv = ["WIPAY_ACCOUNT_NUMBER", "WIPAY_API_KEY", "WIPAY_ENV", "WIPAY_FEE_STRUCTURE"];
  for (const key of requiredEnv) {
    if (!process.env[key]) {
      logWarn("wipay_full_start_missing_env", { missing: key });
  ...
  if (!/^\d+$/.test(accountNumber)) {
    logWarn("wipay_full_start_invalid_account_number");
  ...
  if (!["sandbox", "live"].includes(process.env.WIPAY_ENV ?? "")) {
    logWarn("wipay_full_start_invalid_env", { env: process.env.WIPAY_ENV });
  ...
  // the remaining balance should be handled by the balance flow.
  if (summary.netPaidToDate > 0) {
    return jsonError(409, "already_paid", "Booking already has payments; use Pay Balance");
  }
  ...
  const totalDecimal = formatJmdDecimal(summary.total);
  ...
  try {
    ...
    logError("wipay_full_start_db_insert_failed", error, { bookingId: booking.id, orderId });
  ...
  try {
    ...
    try {
      ...
      logError("wipay_full_start_db_update_failed", dbError, { bookingId: booking.id, orderId, paymentId });
    ...
    logError("wipay_full_start_request_failed", error, {
      bookingId: booking.id,
      orderId,
      amountDecimal: totalDecimal,
      env: process.env.WIPAY_ENV ?? "sandbox",
