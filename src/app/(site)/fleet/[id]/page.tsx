...
export default async function FleetVehicleDetailPage({
  ...
  const { id } = await params;
  const [vehicle, landingContent] = await Promise.all([
    getPublicVehicleByIdentifier(id),
    loadLandingContent(),
  ]);
  ...
  const { fleet, home } = landingContent.content;
  ...
  const detailBookingHref = `${fleet.detailContinueSecondaryCta.href}${
    fleet.detailContinueSecondaryCta.href.includes("?") ? "&" : "?"
  ...
  return (
    <div className="pb-6">
                  ...
                  {vehicle.deposit_cents > 0 ? (
                    <div className="sm:text-right">
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-white/52">
                        {fleet.detailDepositLabel}
                      </p>
                      <p className="mt-1 text-lg font-semibold text-white">{formatJmd(vehicle.deposit_cents)}</p>
                    </div>
                  ) : null}
                </div>
                <p className="mt-5 text-sm leading-7 text-white/70">{home.pricingNote}</p>
      ...
      <PublicSection
        eyebrow={fleet.detailSpecsEyebrow}
        title={fleet.detailSpecsTitle}
        description={fleet.detailSpecsDescription}
      >
        <div className="grid gap-5 sm:gap-6 lg:grid-cols-[1.08fr_0.92fr]">
          ...
          <article className="rounded-[1.9rem] border border-[var(--ccr-border)] bg-[var(--ccr-surface)] p-5 shadow-[0_18px_56px_rgba(15,23,42,0.07)] sm:p-6">
            ...
            <div className="mt-6 rounded-[1.5rem] bg-[var(--ccr-surface-soft)] p-4 sm:p-5">
              <p className="text-sm font-semibold text-[var(--ccr-text)]">{fleet.detailTravelNoteTitle}</p>
              <p className="mt-2 text-sm leading-7 text-[var(--ccr-muted)]">
                {fleet.detailTravelNoteDescription}
              </p>
            </div>
          </article>
        </div>
      ...
      <PublicCtaBand
        eyebrow={fleet.detailContinueEyebrow}
        title={fleet.detailContinueTitle}
        description={fleet.detailContinueDescription}
        primaryAction={fleet.detailContinuePrimaryCta}
        secondaryAction={{
          ...fleet.detailContinueSecondaryCta,
          href: detailBookingHref,
        }}
      />
    </div>
  );
