    ...
    <div className="pb-6">
      <section className="relative overflow-hidden border-b border-[var(--ccr-border)] bg-[var(--ccr-primary)] text-white">
        ...
        <Container className="relative py-10 md:py-16">
          ...
          <div className="mt-6 grid gap-10 xl:grid-cols-[1.08fr_0.92fr] xl:items-start">
            <div>
              <div className="relative h-[320px] overflow-hidden rounded-[2rem] border border-white/10 bg-black/10 shadow-[0_30px_90px_rgba(0,0,0,0.24)] md:h-[460px]">
                <Image
              ...
              {vehicle.images.length > 1 ? (
                <div className="mt-4 grid grid-cols-3 gap-3">
                  {vehicle.images.slice(0, 3).map((image, index) => (
                    <div
                      key={`${image}-${index}`}
                      className="relative h-28 overflow-hidden rounded-[1.4rem] border border-white/10 bg-black/10 md:h-36"
                    >
                      <Image
                        src={image}
            ...
            <div className="xl:sticky xl:top-24">
              ...
              <div className="mt-6 grid grid-cols-3 gap-2 text-sm text-white/72">
                <div className="rounded-full border border-white/12 bg-white/6 px-3 py-3 text-center">
              ...
              <div className="mt-8 rounded-[1.9rem] border border-white/12 bg-white/7 p-6 shadow-[0_20px_80px_rgba(0,0,0,0.2)] backdrop-blur-sm">
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  <Button href={`/book?vehicle=${vehicle.id}`} className="bg-[var(--ccr-accent)] text-[var(--ccr-primary)] hover:bg-[#ffd588]">
                    Reserve This Car
                  </Button>
                  <Button
        ...
        <div className="grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {detailSpecs.map((item) => (
              <article
                key={item.label}
                className="rounded-[1.5rem] border border-[var(--ccr-border)] bg-[var(--ccr-surface)] p-5 shadow-[0_14px_42px_rgba(15,23,42,0.05)]"
              >
          ...
          <article className="rounded-[1.9rem] border border-[var(--ccr-border)] bg-[var(--ccr-surface)] p-6 shadow-[0_18px_56px_rgba(15,23,42,0.07)]">
            ...
            <div className="mt-6 rounded-[1.5rem] bg-[var(--ccr-surface-soft)] p-5">
