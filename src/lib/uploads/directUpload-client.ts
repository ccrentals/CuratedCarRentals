...
function confirmEligibleFiles(files: File[]) {
  const evaluations = files.map((file, index) => ({
    file,
    eligibility: index >= MAX_DIRECT_IMAGES_PER_SELECTION
...
export async function selectAndUploadDirectImages(options: DirectUploadOptions) {
  ...
  const files = confirmEligibleFiles(selected);
