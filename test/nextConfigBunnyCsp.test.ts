...
test("direct image upload gateway config accepts only an HTTPS origin", () => {
  assert.deepEqual(
    getConfiguredDirectImageUploadGateway("https://ccr-production-image-upload-gateway.example"),
    {
      origin: "https://ccr-production-image-upload-gateway.example",
      hostname: "ccr-production-image-upload-gateway.example",
    },
  );
  assert.throws(
    () => getConfiguredDirectImageUploadGateway("http://ccr-production-image-upload-gateway.example"),
